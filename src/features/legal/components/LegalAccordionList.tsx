...
interface LegalAccordionItemProps {
  item: LegalDocumentItem;
  isOpen: boolean;
  onToggle: () => void;
}

function LegalAccordionItem({ item, isOpen, onToggle }: LegalAccordionItemProps) {
        ...
        <View style={styles.termBody}>
          ...
          {item.sections?.map((section) => (
            <View key={section.heading} style={styles.subsection}>
              <Typography variant="body2" style={styles.subsectionHeading}>
                {section.heading}
              </Typography>
              {section.content ? (
                <Typography variant="body2" color={colors.textSecondary} style={styles.paragraph}>
          ...
          {item.bullets?.map((bullet, idx) => (
            <View key={idx} style={styles.bulletRow}>
              <Typography variant="body2" color={colors.textSecondary} style={styles.bulletMarker}>
                •
              </Typography>
              {typeof bullet === 'string' ? (
...
interface LegalAccordionListProps {
  items: LegalDocumentItem[];
}

export function LegalAccordionList({ items }: LegalAccordionListProps) {
  const { spacing } = useTheme();
  const [expanded, setExpanded] = useState<Record<string, boolean>>(
    items.length > 0 ? { [items[0].title]: true } : {},
  );
